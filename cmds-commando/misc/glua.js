...
module.exports = class GLuaLookupCommand extends Command {
	constructor(client) {
		super(client, {
			name: 'glua',
			group: 'misc',
...
function build_rich(row){
	...
	while ((m = regex_arg.exec(html)) !== null){
		if (m.index === regex_arg.lastIndex){regex_arg.lastIndex++;}
		if (m === null){continue;}
		let argType = regex_arg_type.exec(m[1])[2]; regex_arg_type.lastIndex = 0;

		let argName = regex_arg_name.exec(m[1])[1]; regex_arg_name.lastIndex = 0;
		let argDefault = regex_arg_name.exec(m[1])[3]; regex_arg_name.lastIndex = 0;
		let argDesc = regex_arg_desc.exec(m[1])[1].replace(/<a(.*?)>(.*?)<\/a>/g, "$2"); regex_arg_desc.lastIndex = 0;
